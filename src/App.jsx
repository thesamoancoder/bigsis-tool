...
const C = {
  navy:      "#1B2C4A", navyLight: "#243A61", gold: "#D4AF6A",
  goldLight: "#F7F1E4", goldDark:  "#9A7A3A", ice:  "#E3ECF1",
  iceDark:   "#C2D5DF", white:     "#FFFFFF",  bg:   "#F4F7F9",
  text:      "#1A1A2E", muted:     "#6B7A8D",  success: "#2E7D32",
  successBg: "#E8F5E9", warn:      "#B45309",  warnBg:  "#FEF3C7",
  danger:    "#C62828", dangerBg:  "#FFEBEE",
};

const BUREAUS = [
  { value: "", label: "Select your bureau..." },
  { value: "Equifax AU",  label: "Equifax — Australia" },
  { value: "Experian AU", label: "Experian — Australia" },
  { value: "Equifax NZ",  label: "Equifax — New Zealand" },
  { value: "Experian NZ", label: "Experian — New Zealand" },
  { value: "Centrix NZ",  label: "Centrix — New Zealand" },
];
...
function Badge({ strategy }) {
  ...
  return (
    <span style={{ display:"inline-block", fontSize:11, fontWeight:700,
      padding:"2px 9px", borderRadius:99, background:s.bg, color:s.color, letterSpacing:"0.04em" }}>
      {s.code}
    </span>
  );
}
...
function StatusDot({ status }) {
  ...
  return <span style={{ display:"inline-block", width:8, height:8, borderRadius:"50%",
    background: map[status] || C.muted, marginRight:6, flexShrink:0, marginTop:4 }} />;
}

function Card({ children, style = {}, ...rest }) {
  return (
    <div style={{ background:C.white, borderRadius:12, border:`1px solid ${C.iceDark}`,
      padding:"20px 24px", ...style }} {...rest}>
      {children}
    </div>
  );
}
...
function GoldBox({ label, children }) {
  return (
    <div style={{ background:C.goldLight, borderLeft:`4px solid ${C.gold}`,
      borderRadius:"0 8px 8px 0", padding:"14px 18px", margin:"12px 0" }}>
      {label && <div style={{ fontWeight:700, fontSize:12, color:C.goldDark, marginBottom:4,
...
function NavyBox({ children }) {
  return (
    <div style={{ background:C.navy, borderRadius:10, padding:"16px 20px", margin:"12px 0" }}>
...
export default function App() {
  const [step, setStep]           = useState("upload"); // upload | bureau | analysing | results | tracker
  ...
  // Step 1 — read file, go to bureau selection screen
  ...
  // Step 2 — bureau confirmed, run analysis
  async function handleBureauConfirm() {
    if (!bureau) {
      setBureauError("Please select a bureau before continuing.");
      return;
    }
    setBureauError("");
    const isLargeFile = pendingFile && pendingFile.size > 1.5 * 1024 * 1024;
    setStep("analysing");
    setProgress(isLargeFile
      ? "This is a longer report — give us a little extra time (up to 40 seconds)..."
      : "Reading your credit file...");
    await analyseWithClaude(pendingBase64, bureau);
  }

  async function analyseWithClaude(base64, selectedBureau) {
    try {
...
The customer has told you this report is from: ${selectedBureau}. Use this to inform how you read the format and scoring scale.
...
Status guide:
- "flag" = negative entry that should be disputed
- "warn" = entry worth watching or querying
- "ok"   = positive or neutral entry
...
Be thorough but efficient. Skip pages containing only generic definitions, disclaimers, or explanations — focus on pages with real entries (personal details, enquiries, accounts, defaults, judgements). Extract every real entry you can find.`;
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text",     text: "Please analyse this credit report and return the structured JSON as specified." }
            ]
          }]
        })
      });
      ...
      const rows = (parsed.entries || []).map(e => ({
        ...e, disputeSent: false, disputeDate: "", followUpDate: "", outcome: "Pending", notes2: "",
      }));
      setTrackerRows(rows);
      setStep("results");
        ...
        <div style={{ background:C.navy }}>
          <div style={{ maxWidth:700, margin:"0 auto", padding:"24px 24px 20px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
              <div style={{ width:36, height:36, borderRadius:8, background:C.gold, display:"flex",
                alignItems:"center", justifyContent:"center", fontSize:18, fontWeight:900, color:C.navy }}>B</div>
              <div>
            ...
            <div style={{ fontSize:13, color:C.ice, marginTop:8, lineHeight:1.5 }}>
              Upload your credit report PDF — the AI reads it, extracts every entry, and tells you exactly which strategy to use for each one.
            </div>
          </div>
        </div>

        <div style={{ maxWidth:700, margin:"0 auto", padding:"32px 24px" }}>
          <Card style={{ border:`2px dashed ${C.iceDark}`, textAlign:"center", padding:"40px 24px", cursor:"pointer" }}
            ...
            <div style={{ display:"inline-block", background:C.navy, color:C.white,
              padding:"10px 28px", borderRadius:8, fontWeight:600, fontSize:14 }}>Choose File</div>
          </Card>

          {error && (
            <div style={{ background:C.dangerBg, border:`1px solid #FFCDD2`, borderRadius:8,
              padding:"12px 16px", marginTop:16, color:C.danger, fontSize:13 }}>⚠️ {error}</div>
          )}
        ...
        <div style={{ maxWidth:700, margin:"0 auto", padding:"40px 24px" }}>
          <Card>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, background:C.successBg,
              border:`1px solid #A5D6A7`, borderRadius:8, padding:"10px 14px", marginBottom:24 }}>
              <span style={{ fontSize:18 }}>✅</span>
              <div>
                <div style={{ fontSize:13, fontWeight:600, color:C.success }}>File uploaded successfully</div>
                <div style={{ fontSize:12, color:"#2E7D32" }}>{fileName}</div>
              </div>
            </div>

            <div style={{ fontSize:18, fontWeight:700, color:C.navy, marginBottom:6 }}>
              One quick question before we start
            </div>
            <div style={{ fontSize:14, color:C.muted, marginBottom:24, lineHeight:1.6 }}>
              Which bureau did you download this report from? This helps the AI read the correct format and scoring scale for your file.
            </div>

            <div style={{ marginBottom:8 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.navy, display:"block", marginBottom:8 }}>
                Select your credit bureau
              </label>
              <select
                value={bureau}
                onChange={e => { setBureau(e.target.value); setBureauError(""); }}
                style={{ width:"100%", padding:"12px 14px", borderRadius:8, fontSize:14, fontFamily:"Arial, sans-serif",
                  color: bureau ? C.text : C.muted, background:C.white,
                  border:`2px solid ${bureauError ? C.danger : bureau ? C.navy : C.iceDark}`,
                  outline:"none", cursor:"pointer", boxSizing:"border-box" }}>
                {BUREAUS.map(b => (
                  <option key={b.value} value={b.value} disabled={b.value === ""} style={{ color: b.value ? C.text : C.muted }}>
                    {b.label}
                  </option>
                ))}
              </select>
              {bureauError && (
                <div style={{ fontSize:12, color:C.danger, marginTop:6 }}>⚠️ {bureauError}</div>
              )}
            </div>

            <div style={{ marginTop:24, display:"flex", gap:10 }}>
              <button
                onClick={handleBureauConfirm}
                style={{ flex:1, padding:"14px", background:C.navy, color:C.white, border:"none",
                  borderRadius:10, fontSize:15, fontWeight:700, cursor:"pointer" }}>
                Analyse My Credit File →
              </button>
              <button
                onClick={() => { setStep("upload"); setError(""); }}
                style={{ padding:"14px 20px", background:"transparent", color:C.muted, border:`1px solid ${C.iceDark}`,
          ...
          <GoldBox label="Not sure which bureau?">
            Check the logo or header on the first page of your PDF. Equifax reports usually say "Equifax" in blue. Experian reports say "Experian" in purple. Centrix reports say "Centrix" and are New Zealand only. If you downloaded from GetCreditScore.com.au, that is Equifax AU.
          </GoldBox>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"Arial, sans-serif",
        ...
        <div style={{ textAlign:"center", padding:40, maxWidth:500 }}>
          <div style={{ width:64, height:64, borderRadius:"50%", background:C.navy, display:"flex",
            alignItems:"center", justifyContent:"center", margin:"0 auto 20px", fontSize:28 }}>
          ...
          <div style={{ fontSize:12, color:C.iceDark, marginTop:4 }}>{fileName} · {bureau}</div>
          <div style={{ fontSize:12, color:C.muted, marginTop:16, lineHeight:1.6 }}>
            This usually takes 15–40 seconds depending on how many entries are on your file. Please don't close this tab.
          </div>
          ...
          <div style={{ maxWidth:900, margin:"0 auto", padding:"20px 24px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <div style={{ width:32, height:32, borderRadius:7, background:C.gold, display:"flex",
                  alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:900, color:C.navy }}>B</div>
                <div>
                  <div style={{ fontSize:10, color:C.gold, letterSpacing:"0.1em", textTransform:"uppercase" }}>Big Sis Credit Hacks</div>
                  <div style={{ fontSize:15, fontWeight:700, color:C.white }}>Credit File Analysis · {analysis.bureau}</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={() => setStep("results")}
                  style={{ padding:"7px 16px", borderRadius:7, border:"none", cursor:"pointer", fontSize:13, fontWeight:600,
                    background: step==="results" ? C.gold : "transparent", color: step==="results" ? C.navy : C.ice }}>
                  Analysis
                </button>
                <button onClick={() => setStep("tracker")}
                  style={{ padding:"7px 16px", borderRadius:7, border:"none", cursor:"pointer", fontSize:13, fontWeight:600,
                    background: step==="tracker" ? C.gold : "transparent", color: step==="tracker" ? C.navy : C.ice }}>
                  Tracker ({disputed.length}/{trackerRows.length})
                </button>
                <button onClick={() => { setStep("upload"); setAnalysis(null); setFileName(""); setError(""); setBureau(""); }}
                  style={{ padding:"7px 16px", borderRadius:7, border:`1px solid rgba(255,255,255,0.2)`,
                    cursor:"pointer", fontSize:13, color:C.ice, background:"transparent" }}>
                  New File
                </button>
              </div>
            </div>
            ...
            <>
              <div style={{ display:"grid", gridTemplateColumns: analysis.score !== "Unknown" ? "160px 1fr" : "1fr", gap:14, marginBottom:20 }}>
                ...
                <Card>
                  ...
                  {analysis.quickWins && (
                    <div style={{ background:C.successBg, borderLeft:`3px solid ${C.success}`,
                      borderRadius:"0 6px 6px 0", padding:"10px 14px", fontSize:13, color:"#1B5E20" }}>
              ...
              {analysis.warnings?.length > 0 && (
                <div style={{ background:C.warnBg, border:`1px solid #FCD34D`, borderRadius:10,
                  padding:"14px 18px", marginBottom:20 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:C.warn, marginBottom:8,
                    textTransform:"uppercase", letterSpacing:"0.06em" }}>⚠️ Important Notices</div>
              ...
              <div style={{ fontSize:13, fontWeight:700, color:C.navy, marginBottom:10,
                textTransform:"uppercase", letterSpacing:"0.06em" }}>All Entries</div>

              <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:16 }}>
                {Object.entries(STRATEGY_MAP).slice(0,5).map(([name,s]) => (
                  <span key={name} style={{ fontSize:11, padding:"3px 10px", borderRadius:99,
                    background:s.bg, color:s.color, fontWeight:600 }}>{s.code} — {name}</span>
                ))}
              </div>
                    ...
                    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:13, color:"#444", marginBottom:6 }}>
                          <strong style={{ color:C.navy }}>Status:</strong> {entry.statusReason}
                        </div>
                        {entry.suggestedStrategy !== "Monitor / No Action Needed" && (
                          <div style={{ fontSize:13, color:"#444" }}>
                            <strong style={{ color:C.navy }}>Strategy:</strong> {entry.strategyReason}
                          </div>
                        )}
              ...
              <div style={{ marginTop:24 }}>
                <button onClick={() => setStep("tracker")}
                  style={{ width:"100%", padding:"14px", background:C.navy, color:C.white, border:"none",
                    borderRadius:10, fontSize:15, fontWeight:700, cursor:"pointer" }}>
            ...
            <>
              ...
              <Card style={{ marginBottom:20 }}>
                ...
                <div style={{ height:8, background:C.bg, borderRadius:99, overflow:"hidden" }}>
                  <div style={{ height:"100%", background:`linear-gradient(90deg, ${C.navy}, ${C.gold})`,
                    borderRadius:99,
                    width:`${trackerRows.filter(r=>r.status!=="ok").length > 0 ? (disputed.length/trackerRows.filter(r=>r.status!=="ok").length)*100 : 0}%`,
                ...
                return (
                  <div key={statusGroup} style={{ marginBottom:24 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:C.muted, textTransform:"uppercase",
                      letterSpacing:"0.07em", marginBottom:10 }}>{groupLabel}</div>
                    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                      {groupRows.map(row => (
                        <Card key={row.id} style={{ opacity: row.outcome==="Removed ✅" ? 0.6 : 1, transition:"opacity 0.3s" }}>
                          <div style={{ display:"flex", gap:14, alignItems:"flex-start", flexWrap:"wrap" }}>
                            <div onClick={() => updateRow(row.id,"disputeSent",!row.disputeSent)}
                              style={{ width:22, height:22, borderRadius:6, flexShrink:0, marginTop:2,
                                border:`2px solid ${row.disputeSent ? C.navy : C.iceDark}`,
                                background: row.disputeSent ? C.navy : "transparent",
                            ...
                            <div style={{ flex:1 }}>
                              ...
                              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px,1fr))", gap:8, marginTop:8 }}>
                                ...
                                <div>
                                  ...
                                  <input type="date" value={row.followUpDate}
                              ...
                              <div style={{ marginTop:8 }}>
                                <input type="text" placeholder="Notes (reference number, what they said, next step…)"
                                  value={row.notes2} onChange={e => updateRow(row.id,"notes2",e.target.value)}
